...
class TeacherHelpRequestController extends Controller
{
    public function index(Request $request)
    {
        ...
        return view('teachers.help_requests', compact('helpRequests', 'statusFilter'));
    }
    ...
    public function reply(Request $request, $id)
    {
        ...
        // Create log entry
        $actionDetail = "ครู " . auth()->user()->name . " ตอบกลับคำร้อง";
        ...
        HelpRequestLog::create([
            ...
            'note' => $request->action_note ?? $request->admin_reply, // Use action note if provided, otherwise default to reply
        ]);

        // Send notification to Student
        $msg = "คำร้องขอความช่วยเหลือของคุณได้รับการตอบกลับโดยครู " . auth()->user()->name . " (สถานะ: " . $this->getStatusThai($newStatus) . ")";
        HelpNotification::sendNotification($helpRequest->user_id, $helpRequest->id, $msg);

        return redirect()->route('teachers.help.index')->with('success', 'บันทึกการดำเนินการและตอบกลับผู้เรียนเรียบร้อยแล้ว');
    }
    ...
    public function updateStatus(Request $request, $id)
    {
        ...
        $actionDetail = "ครู " . auth()->user()->name . " อัปเดตสถานะเป็น " . $this->getStatusThai($newStatus);
        ...
        return redirect()->route('teachers.help.index')->with('success', 'อัปเดตสถานะความช่วยเหลือเรียบร้อยแล้ว');
    }
