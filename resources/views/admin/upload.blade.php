<x-admin-layout>
    <div class="p-4 sm:ml-64">
        ...
        {{-- Clear Table --}}
        <div class="pt-2 border-2 border-gray-200 border-dashed rounded-lg dark:border-gray-700 mt-2">
            <!-- แสดงข้อความ error -->
            @if(session('error'))
            ...
            <div id="alert-container" class="hidden rounded-lg"></div>
        ...
        forms.forEach(form => {
            form.addEventListener('submit', function (e) {
                ...
                .then(data => {
                    ...
                    alertContainer.innerHTML = `<div class="bg-${data.success ? 'green' : 'red'}-600 text-white p-4 rounded-lg">${data.message}</div>`;
                })
                .catch(any => {
                    modal.classList.add('hidden');
                    alertContainer.classList.remove('hidden');
                    alertContainer.innerHTML = `<div id="alert-border-3" class="flex items-center p-4 mb-4 text-green-800 border-t-4 border-green-300 bg-green-50 dark:text-green-400 dark:bg-gray-800 dark:border-green-800" role="alert">
