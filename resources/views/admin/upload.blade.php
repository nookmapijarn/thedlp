    ...
    <div class="p-4 sm:ml-64">
        <div class="p-4 border-2 border-gray-200 border-dashed rounded-lg dark:border-gray-700 mt-14">
            ...
            <div class="grid justify-center p-3">
                <ol class="list-decimal p-8 bg-gray-200 block max-w-sm p-6 bg-yellow-200 border border-gray-200 rounded-lg shadow">
                    <strong>อธิบายขั้นตอนการนำเข้าข้อมูล</strong>
                    <li>เตรียมไฟล์ Backup เช่น ITW51DATA.zip (สถานศึกษาขนาดใหญ่หรือขนาดไฟล์เกิน 50MB ให้พิจารณา Backup และนำเข้าทีละระดับ)</li>
                    <li>เลือกไฟล์ Backup ที่ต้องการและกดปุ่ม upload</li>
                    <li>ระยะเวลาอัพโหลดประมาณ 1-10 นาที ขึ้นอยู่กับขนาดของข้อมูล และ ความเร็วอินเทอร์เน็ต</li>
                    <li>ระบบจะอ่านข้อมูลเฉพาะที่มีการแก้ไขจากครั้งก่อนเท่านั้น หากไม่มีการแก้ไขจะไม่อัพโหลด</li>
                    <li>ปุ่มล้างข้อมูลทั้งหมด และ ล้างตารางเวลาอัพเดท (ใช้ในกรณีต้องการล้างข้อมูลเดิมทั้งหมดและอัพโหลดใหม่ทั้งหมด)</li>
                </ol>
            </div>

            <div class="grid justify-start mx-auto mt-4 max-w-4xl sm:mt-6">
                {{-- Clear Table --}}
                <form id="yourForm" action="{{ route('clearTable') }}" method="POST" class="">
                    @csrf
                ...
                <form id="yourForm" action="{{ route('clearDateModifiled') }}" method="POST" class="">
                    @csrf
...
<div id="modal" class="hidden fixed inset-0 bg-gray-800 bg-opacity-50 flex items-center justify-center z-50">
    <ul class="max-w-md space-y-2 text-gray-100 list-inside dark:text-gray-400">
        <li class="flex items-center">
            ...
            กำลังดำเนินการประมาณ 1-10 นาที... 
        </li>
    </ul>
</div>
