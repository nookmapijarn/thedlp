<nav x-data="{ open: false }" class="bg-opacity-50 background-animate bg-gradient-to-r from-purple-500 via-violet-800 to-purple-500 shadow-10 border-b border-gray-100 dark:bg-gray-800 sticky top-0 z-50">
        ...
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                <div class="shrink-0 flex items-center">
                    ...
                    <strong class="ml-2 text-gray-100 text-lg tracking-wide"> ช่วยเหลือผู้เรียน </strong>

                </div>

                <!-- Navigation Links -->
                <div class="hidden space-x-8 sm:-my-px sm:ml-10 sm:flex text-lg">
                    <x-nav-link :href="route('สมัครเรียน')" :active="request()->routeIs('สมัครเรียน')" class="text-white">
                        {{ __('สมัครเรียน') }}
                    </x-nav-link>
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden  text-white">
        <div class="pt-2 pb-3 space-y-1">
            <x-responsive-nav-link :href="route('สมัครเรียน')" :active="request()->routeIs('สมัครเรียน')" class="{{request()->routeIs('สมัครเรียน') ? 'text-indigo-800' : 'text-white'}}">
                {{ __('สมัครเรียน') }}
